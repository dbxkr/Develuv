...
public interface MatchingListMapper {
    ...
    @Select("select user_id, user_name, user_gender, user_nbti, user_address, user_profile " +
            "from users where user_address like concat('%',#{user_city},'%') and user_gender!=#{gender} order by RAND() limit 100")
    ...
    @Select("select user_id " +
            "from users where user_address like concat('%',#{user_city},'%') and user_gender!=#{gender} order by user_heart desc limit 50")
